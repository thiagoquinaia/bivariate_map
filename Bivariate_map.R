...
map <- ggplot() +
  ...
  geom_raster(data = data, mapping = aes(x = x, y = y, fill = bi_class), color = NA, linewidth = 0.1, show.legend = TRUE) +
...
print(class_means %>% 
        arrange(temp_mean, ppt_mean) %>% 
        select(bi_class, temp_mean, ppt_mean))
...
legend_data <- data.frame(
  ...
  y = rep(1:4, times = 4),
  fill = bi_colors,
  label = paste0("T:", class_means$temp_mean, "°C\nP:", class_means$ppt_mean, "mm")
)
...
finalPlot <- ggdraw() +
  ...
  draw_plot(legend, 0.65, 0.05, 0.35, 0.35)
